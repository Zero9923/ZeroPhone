...
window.currentUploadedAvatarUrl = null; 
window.editingCharId = null; 
window.isSearchOpen = false;
window.isReceiving = false; 

window.gradients = ['from-blue-400 to-indigo-500','from-pink-400 to-rose-500','from-amber-400 to-orange-500','from-emerald-400 to-teal-500'];
...
window.pressTimer = null; window.isLongPressTrigged = false; window.deleteTarget = { type: null, id: null };

window.startPress = function(e, type, id) {
    window.isLongPressTrigged = false;
    window.pressTimer = setTimeout(() => { window.isLongPressTrigged = true; window.showDeleteModal(type, id); }, 600); 
};
window.cancelPress = function() { if(window.pressTimer) clearTimeout(window.pressTimer); };
...
window.executeDelete = function() {
    const { type, id } = window.deleteTarget; if(!id) return;
    if(type === 'character') {
        ...
        if(window.currentChatId === id) window.currentChatId = null;
    } else if (type === 'chat') {
        window.chats = window.chats.filter(c => c.id !== id);
        window.chatHistories[id] = []; 
        if(window.currentChatId === id) document.getElementById('chat-messages').innerHTML = '';
    }
...
window.toggleSearch = function() {
    window.isSearchOpen = !window.isSearchOpen; const container = document.getElementById('search-container'); const input = document.getElementById('search-input'); const avatars = document.getElementById('top-avatars-container');
    if(window.isSearchOpen) { container.classList.remove('max-h-0', 'opacity-0'); container.classList.add('max-h-16', 'opacity-100'); avatars.classList.add('opacity-0', 'scale-y-0', 'h-0', 'overflow-hidden'); setTimeout(() => input.focus(), 300); } 
...
window.handleSearch = function(query) {
    ...
    if (charMatches.length > 0) { list.innerHTML = `<div class="px-3 pb-2 pt-2 text-[10px] font-bold text-zinc-400 uppercase tracking-widest">Characters Found</div>` + charMatches.map(c => window.renderSearchItem(c, 'Tap to start chatting')).join(''); } 
        ...
        const recordMatches = window.chats.filter(c => c.prompt && c.prompt.toLowerCase().includes(q));
...
window.saveCharacter = function() {
    ...
    if (window.editingCharId) { const char = window.characters.find(c => c.id === window.editingCharId); if (char) { char.name = name; char.gender = gender; char.age = age; char.prompt = prompt; char.avatarUrl = window.currentUploadedAvatarUrl; } } 
    else { const color = window.gradients[window.characters.length % window.gradients.length]; const newChar = { id: Date.now(), name, gender, age, prompt, color, avatarUrl: window.currentUploadedAvatarUrl }; window.characters.push(newChar); window.chatHistories[newChar.id] = []; }
    window.saveAllLingoData(); document.getElementById('modal-create-form').classList.add('translate-y-full'); window.renderCharacters(); window.renderTopAvatars(); window.renderChatList(); if (!window.editingCharId) window.switchChatTab('chat');
};
...
window.renderCharacters = function() {
    ...
    <div 
        ...
        onclick="if(!window.isLongPressTrigged) window.editCharacter(${c.id})" 
...
window.renderChatList = function() {
    ...
    list.innerHTML = window.chats.map(c => { 
        ...
        <div 
            ...
            onclick="if(!window.isLongPressTrigged) window.openChatRoom(${c.id})"
            style="-webkit-touch-callout: none; user-select: none;"
