...
const chatRoomUI = `
    ...
    <div id="page-chat-room" class="absolute inset-0 z-[70] bg-[#F2F2F7] translate-x-full transition-transform duration-300 flex flex-col overflow-hidden">
            ...
            <div id="chat-feature-menu" class="max-h-0 overflow-hidden bg-[#F2F2F7] transition-all duration-300 border-t border-zinc-200">
                <div class="px-6 py-6 pb-12 grid grid-cols-4 gap-y-6 gap-x-4 place-items-start">
                    <button type="button" onclick="window.regenerateReply()" class="flex flex-col items-center gap-2 active:scale-95 transition-transform w-full outline-none">
                        <div class="w-14 h-14 bg-white rounded-2xl flex items-center justify-center shadow-sm text-zinc-800 pointer-events-none">
                        ...
                        <span class="text-[10px] font-semibold text-zinc-500 pointer-events-none">重回</span>
                    </button>
                    <button type="button" onclick="window.openTransferModal()" class="flex flex-col items-center gap-2 active:scale-95 transition-transform w-full outline-none">
                        <div class="w-14 h-14 bg-white rounded-2xl flex items-center justify-center shadow-sm text-[#F59A23] pointer-events-none">
                        ...
                        <span class="text-[10px] font-semibold text-zinc-500 pointer-events-none">转账</span>
                    </button>
                    <button type="button" onclick="window.openRedPacketModal()" class="flex flex-col items-center gap-2 active:scale-95 transition-transform w-full outline-none">
                        <div class="w-14 h-14 bg-white rounded-2xl flex items-center justify-center shadow-sm text-[#F45447] pointer-events-none">
                        ...
                        <span class="text-[10px] font-semibold text-zinc-500 pointer-events-none">红包</span>
                    </button>
                </div>
            </div>
        </div>
        ...
        <!-- 多选模式栏 -->
        <div id="multi-select-bar" class="hidden absolute bottom-0 left-0 w-full bg-white/90 backdrop-blur-md border-t border-zinc-200 px-6 py-4 pb-10 flex items-center justify-between z-30 shadow-lg">
        ...
        <!-- 🌟 心声面板弹窗 -->
        <div id="mind-panel-modal" class="hidden absolute inset-0 z-[100] bg-black/40 backdrop-blur-sm flex items-center justify-center p-6" onclick="window.closeMindPanel()">
            <div class="bg-white rounded-3xl w-full max-w-[300px] p-6 shadow-2xl flex flex-col gap-4 transform scale-95 transition-transform duration-300" id="mind-panel-card" onclick="event.stopPropagation()">
                <div class="flex items-center gap-3 border-b border-zinc-100 pb-4">
                    <div id="mind-avatar" class="w-12 h-12 rounded-full overflow-hidden shadow-sm flex-shrink-0"></div>
                    <div class="flex flex-col"><span class="text-[15px] font-bold text-zinc-900 truncate" id="mind-name">Name</span><span class="text-[10px] text-zinc-400 font-bold tracking-widest mt-0.5">INNER VOICE</span></div>
                </div>
                <div class="flex flex-col gap-3 pt-1">
                    <div class="flex flex-col gap-1"><span class="text-[10px] font-bold text-zinc-400 uppercase">Outfit / 穿着</span><span class="text-[13px] text-zinc-800 font-medium leading-snug" id="mind-outfit">未察觉出特别的穿着</span></div>
                    <div class="flex flex-col gap-1"><span class="text-[10px] font-bold text-zinc-400 uppercase">Location / 位置</span><span class="text-[13px] text-zinc-800 font-medium leading-snug" id="mind-location">当前位置未知</span></div>
                    <div class="flex flex-col gap-1 bg-zinc-50 p-3 rounded-2xl border border-zinc-100 mt-1"><span class="text-[10px] font-bold text-zinc-400 uppercase mb-0.5">Inner Voice / 心声</span><span class="text-[13px] text-zinc-900 font-semibold italic leading-snug" id="mind-voice">对方的心思深不可测...</span></div>
                </div>
            </div>
        </div>
        ...
        <div id="msg-edit-modal" class="hidden absolute inset-0 z-[100] bg-black/50 backdrop-blur-sm flex items-center justify-center p-6">
            <div class="bg-white rounded-3xl w-full max-w-[320px] p-5 shadow-2xl flex flex-col gap-4">
                ...
                <div class="flex gap-3">
                    <button onclick="document.getElementById('msg-edit-modal').classList.add('hidden')" class="flex-1 py-3 bg-zinc-100 text-zinc-600 rounded-xl text-[13px] font-bold">Cancel</button>
                    <button onclick="window.saveMsgEdit()" class="flex-1 py-3 bg-black text-white rounded-xl text-[13px] font-bold shadow-md">Save</button>
                </div>
            </div>
        </div>
        ...
        <div id="forward-modal" class="hidden absolute inset-0 z-[100] bg-black/50 backdrop-blur-sm flex items-end justify-center" onclick="document.getElementById('forward-modal').classList.add('hidden')">
            <div class="bg-white w-full rounded-t-3xl p-5 pb-10 max-h-[70%] overflow-y-auto" onclick="event.stopPropagation()">
                <h3 class="text-[15px] font-bold text-zinc-900 text-center mb-4">Forward To...</h3>
                <div id="forward-char-list" class="flex flex-col gap-2"></div>
            </div>
        </div>
...
window.openChatRoom = function(charId) { 
    window.currentChatId = charId; 
    window.isReceiving = false; 
...
window.closeChatRoom = function() { 
    document.getElementById('page-chat-room').classList.add('translate-x-full'); 
    window.hideChatMenu();
...
// 🌟 心声面板功能
window.showMindPanel = function() {
    const char = window.characters.find(c => c.id === window.currentChatId); if(!char) return;
    const avatarEl = document.getElementById('mind-avatar');
    ...
    document.getElementById('mind-outfit').innerText = char.mindState?.outfit || "未察觉出特别的穿着";
    document.getElementById('mind-location').innerText = char.mindState?.location || "当前位置未知";
    document.getElementById('mind-voice').innerText = char.mindState?.voice || "对方的心思深不可测...";
    const modal = document.getElementById('mind-panel-modal'); const card = document.getElementById('mind-panel-card');
    modal.classList.remove('hidden'); setTimeout(() => card.classList.remove('scale-95'), 10);
};

window.closeMindPanel = function() { 
    const modal = document.getElementById('mind-panel-modal'); const card = document.getElementById('mind-panel-card'); 
    card.classList.add('scale-95'); setTimeout(() => modal.classList.add('hidden'), 300); 
};
...
window.sendTransfer = function() { const amount = document.getElementById('transfer-amount').value; const memo = document.getElementById('transfer-memo').value || '转账'; if(!amount || amount <= 0) return alert("请输入正确金额"); if(!window.chatHistories[window.currentChatId]) window.chatHistories[window.currentChatId] = []; window.chatHistories[window.currentChatId].push({ id: window.generateId(), role: 'user', type: 'transfer', amount: amount, text: memo, isReceived: false, isRefunded: false }); window.saveAllLingoData(); window.renderChatMessages(); document.getElementById('transfer-modal').classList.add('hidden'); };
window.sendRedPacket = function() { const amount = document.getElementById('redpacket-amount').value; const memo = document.getElementById('redpacket-memo').value || '恭喜发财，大吉大利'; if(!amount || amount <= 0) return alert("请输入正确金额"); if(!window.chatHistories[window.currentChatId]) window.chatHistories[window.currentChatId] = []; window.chatHistories[window.currentChatId].push({ id: window.generateId(), role: 'user', type: 'redpacket', amount: amount, text: memo, isReceived: false }); window.saveAllLingoData(); window.renderChatMessages(); document.getElementById('redpacket-modal').classList.add('hidden'); };
...
window.handleCardClick = function(msgId) {
    ...
    if (msg.type === 'transfer') {
        ...
        if (msg.isReceived) {
            ...
            setTimeout(()=> detailModal.classList.remove('translate-y-full'), 10);
        } else {
...
window.renderChatMessages = function() {
    ...
    // 头像点击绑定弹窗
    const charAvatarHTML = char.avatarUrl ? `<img src="${char.avatarUrl}" onclick="!window.isMultiSelecting && window.showMindPanel()" class="w-8 h-8 rounded-full object-cover flex-shrink-0 shadow-sm cursor-pointer active:scale-90 transition-transform pointer-events-auto" />` : `<div onclick="!window.isMultiSelecting && window.showMindPanel()" class="w-8 h-8 rounded-full bg-gradient-to-tr ${char.color} flex items-center justify-center text-white flex-shrink-0 shadow-sm text-xs font-bold cursor-pointer active:scale-90 transition-transform pointer-events-auto">${char.name.charAt(0).toUpperCase()}</div>`;
    ...
    container.innerHTML = history.map((msg, idx) => {
        ...
        // 🌟 微信风格的缩略引用气泡 (增加了 max-w-[180px] 和 truncate 进行截断)
        const quoteHtmlAi = msg.quote ? `<div class="text-[11px] text-zinc-400 bg-black/5 px-2.5 py-1.5 rounded-lg mt-1 ml-1 self-start max-w-[180px] truncate shadow-sm font-medium">引用: ${msg.quote}</div>` : '';
        const quoteHtmlUser = msg.quote ? `<div class="text-[11px] text-zinc-400 bg-black/5 px-2.5 py-1.5 rounded-lg mt-1 mr-1 self-end max-w-[180px] truncate shadow-sm font-medium text-right">引用: ${msg.quote}</div>` : '';
...
window.multiAction = function(action) {
    ...
    if(action === 'del') {
        window.chatHistories[window.currentChatId] = window.chatHistories[window.currentChatId].filter(m => !window.selectedMsgIds.includes(m.id));
        window.saveAllLingoData(); window.endMultiSelect();
    } else if (action === 'fav') {
        alert("已收藏到 我的 页面！(占位功能)"); window.endMultiSelect();
    }
};
...
window.openForwardModal = function() {
    ...
    list.innerHTML = window.characters.map(c => `
        <div onclick="window.executeForward(${c.id})" class="p-4 border border-zinc-100 rounded-2xl flex items-center gap-3 cursor-pointer active:bg-zinc-50 shadow-sm">
            ${c.avatarUrl ? `<img src="${c.avatarUrl}" class="w-10 h-10 rounded-full object-cover"/>` : `<div class="w-10 h-10 rounded-full bg-gradient-to-tr ${c.color} flex items-center justify-center text-white font-bold">${c.name.charAt(0).toUpperCase()}</div>`}
            <span class="font-bold text-zinc-900">${c.name}</span>
        </div>`).join('');
    document.getElementById('forward-modal').classList.remove('hidden');
};

window.executeForward = function(targetId) {
    const msgs = window.chatHistories[window.currentChatId].filter(m => window.selectedMsgIds.includes(m.id)).map(m => {
        return { roleName: m.role === 'user' ? '我' : window.characters.find(c=>c.id===window.currentChatId).name, text: m.isRevoked ? "(撤回了一条消息)" : m.text || "[卡片消息]" };
    });
...
window.openForwardView = function(dataStr) {
    ...
    document.getElementById('forward-view-content').innerHTML = msgs.map(m => `
        <div class="flex flex-col mb-1 border-b border-zinc-100 pb-3">
            <span class="text-[11px] font-bold text-zinc-500 mb-1">${m.roleName}</span>
            <span class="text-[13px] text-zinc-900">${m.text}</span>
        </div>
    `).join('');
    document.getElementById('forward-view-modal').classList.remove('hidden');
    setTimeout(()=> document.getElementById('forward-view-modal').classList.remove('translate-y-full'), 10);
};
...
window.receiveMessage = async function() {
    ...
    const systemPrompt = `身份：${char.name}。性别：${char.gender||'保密'}，年龄：${char.age||'保密'}。
设定：${char.prompt}
规则：
1. 用微信聊天，绝不暴露AI身份，无心理描写。多句话用换行符(\\n)隔开！
2. 【动作指令】(必须单独一行输出，严格按格式)：
- 引用对方：<QUOTE:引用的原话>
- 发转账：<TRANSFER:金额:备注>
- 发红包：<REDPACKET:金额:备注>
- 收转账：<RECEIVE>
- 退转账：<REFUND>
- 领红包：<RECEIVE_REDPACKET>
- 撤回首句：<REVOKE>
3. 【心声追踪】每轮最后必须起三行偷偷输出：
<OUTFIT:现在的穿着>
<LOCATION:现在的位置>
<VOICE:内心真实想法>`;
    ...
    try {
        ...
        // 提取心声面板状态
        let outfitMatch = aiReply.match(/<OUTFIT:\s*(.*?)\s*>/);
        if(outfitMatch) { char.mindState = char.mindState || {}; char.mindState.outfit = outfitMatch[1].trim(); aiReply = aiReply.replace(outfitMatch[0], '').trim(); }
        let locMatch = aiReply.match(/<LOCATION:\s*(.*?)\s*>/);
        if(locMatch) { char.mindState = char.mindState || {}; char.mindState.location = locMatch[1].trim(); aiReply = aiReply.replace(locMatch[0], '').trim(); }
        let voiceMatch = aiReply.match(/<VOICE:\s*(.*?)\s*>/);
        if(voiceMatch) { char.mindState = char.mindState || {}; char.mindState.voice = voiceMatch[1].trim(); aiReply = aiReply.replace(voiceMatch[0], '').trim(); }

        let isRevokedByAI = false; if(aiReply.includes('<REVOKE>')) { isRevokedByAI = true; aiReply = aiReply.replace(/<REVOKE>/g, '').trim(); }
        ...
        for (let i = 0; i < replies.length; i++) {
            ...
            let currentQuote = globalQuoteText; globalQuoteText = null; 
